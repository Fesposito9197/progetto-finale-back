...
<form action="{{route('admin.companies.store')}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        <label for="opening_hours" class="form-label">Project Name</label>
    ...
    <div class="mb-3">
        ...
        <input class="form-control @error('minimum_order') is-invalid @enderror" type="number" placeholder="minimum_order" id="minimum_order" name="minimum_order" value="{{old('minimum_order')}}" required>
        @error('minimum_order')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
    </div>
    <div class="mb-3">
        ...
        @foreach ($typologies as $typology)
            <input type="checkbox" class="form-check-label" name="typologies[]" id="{{$typology->id}} {{ in_array( $technology->id, old('technologies', [])) ? 'checked' : '' }}" value="{{$typology->id}}">
